...
```{r}
# ----- VERIFYING RECODING, IGNORE -----
# unique(df_18_2$RAC1P) # - good
# # unique(df_18_2$SEX) - good
# unique(df_18_2$HISP)
# unique(df_18_2$race_ethnicity)
# ----- IGNORE | VERIFYING RECODE | IGNORE -----
```
...
```{r}
write.csv(df_18_2, 'city_demo_2018_pums.csv', row.names = F)
write.csv(df_19_2, 'city_demo_2019_pums.csv', row.names = F)
write.csv(df_20_2, 'city_demo_2020_pums.csv', row.names = F)
```
...
# female: 
city_female_18 <- sex_18 %>%  filter(SEX =="Female") %>% select(percent) %>% pull()
city_female_19 <- sex_19 %>%  filter(SEX =="Female") %>% select(percent) %>% pull()
city_female_20 <- sex_20 %>%  filter(SEX =="Female") %>% select(percent) %>% pull()

# male: 
city_male_18 <- sex_18 %>%  filter(SEX =="Male") %>% select(percent) %>% pull()
city_male_19 <- sex_19 %>%  filter(SEX =="Male") %>% select(percent) %>% pull()
city_male_20 <- sex_20 %>%  filter(SEX =="Male") %>% select(percent) %>% pull()

# nh white:
city_white_18 <- race_18 %>%  filter(race_ethnicity =="NH White") %>% select(percent) %>% pull()
city_white_19 <- race_19 %>%  filter(race_ethnicity =="NH White") %>% select(percent) %>% pull()
city_white_20 <- race_20 %>%  filter(race_ethnicity =="NH White") %>% select(percent) %>% pull()



city_black <- race %>%  filter(race_ethnicity =="NH Black") %>% select(percent) %>% pull()

city_hispanic <- race %>%  filter(race_ethnicity =="Hispanic or Latino") %>% select(percent) %>% pull()

city_asian <- race %>%  filter(race_ethnicity =="NH Asian") %>% select(percent) %>% pull()

city_sor <- race %>%  filter(race_ethnicity =="NH SOR or Race UCND") %>% select(percent) %>% pull()
```
...
```{r}
# ***TO DO: GET NEW DATA***
# read in 2018 agency diversity csv (from DCAS worker profile)

wp <- read.csv("../data/Agency_Diversity.csv") %>% janitor::clean_names()
```

```{r}
# ***TO DO: FILTER BY AGENCIES***
# GENDER: DOITT
head(wp)
```
...
```{r}
# distance to city diversity calculations
...
# wp$d6 <- sqrt(wp$x_female - city_female)^2
...
write.csv(wp, 'agency_diversity_calculations.csv', row.names = F)
