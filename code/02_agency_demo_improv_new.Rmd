...
```{r}
poor_agency_gender[poor_agency_gender$agency_name == 'DEP',]
1279/(1279+4208+12)
1310/(1310+4226+3+2+17)
1291/(1291+4054+3+3+19)
```
...
cp
cp[cp$gender == 'Female',]
cp$gender2 <- recode(cp$gender, 
                    "Non-binary" = "Other",
                    "Other Gender" = "Other",
                    "Unknown" = "Other")
```
pct change is year to year not compared to 2018 now 
...
p1 <- cp %>%  filter(agency_name =='DEP') %>% 
  ...
  ggtitle("DEP Gender") + xlab("Year") + ylab("Percent Change")

p2 <- cp %>%  filter(agency_name =='DEP') %>% 
  ...
  hrbrthemes::theme_ipsum() +
  ggtitle("DEP Gender") + xlab("Year") + ylab("Relative Difference")
